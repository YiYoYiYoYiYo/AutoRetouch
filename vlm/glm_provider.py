...
class GLMProvider(VLMProvider):
    """GLM-4.1V-Thinking-Flash 云端提供者"""

    def __init__(self, config: GLMConfig | None = None):
        self._cfg = config or GLMConfig()
    ...
    def _parse(self, raw: str) -> EditSuggestion:
        """从模型输出中提取 JSON 并解析为 EditSuggestion"""
    ...
    def _extract_json(text: str) -> str:
        """从文本中提取 JSON 字符串（兼容 markdown 代码块）"""
        # 尝试提取 ```json ... ``` 块
